...
class Color$2 {
  ...
  blend(c, t) {
    ...
    return new Color$2(source.r * (1 - t) + target.r * t, source.g * (1 - t) + target.g * t, source.b * (1 - t) + target.b * t);
  }
  toString() {
...
class Link$1 {
  ...
  toData() {
    const data = {};
    data.n0 = [this.n0.group.id, this.n0.id];
    data.nf = [this.nf.group.id, this.nf.id];
...
class DataPoint$1 {
  ...
  render() {
    ...
    let fillColor;
    if (this.label == 0) fillColor = Color$1.red;else fillColor = Color$1.blue;
...
class DragBehavior$1 {
  constructor(canvas) {
    ...
    canvas.addEventListener("mousedown", this._onDragBegin, {
      passive: false
    });
    canvas.addEventListener("touchstart", this._onDragBegin, {
      passive: false
    });
    this._onDragProgress = this.onDragProgress.bind(this);
    canvas.addEventListener("mousemove", this._onDragProgress, {
      passive: false
    });
    canvas.addEventListener("touchmove", this._onDragProgress, {
      passive: false
    });
...
class LossPlot {
  ...
  push(totalLoss) {
    ...
    if (maxTotalLoss > this.minTopLoss) this.topLoss = maxTotalLoss;else this.topLoss = this.minTopLoss;
    this.render();
  }
  render() {
    const ctx = this.ctx;
